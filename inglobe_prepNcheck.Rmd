...
if (Sys.info()["nodename"] == "DESKTOP-MMF34H4" ) {
  #datad="C:\\Users\\rmartin7\\OneDrive - WBG\\projects\\PRINZGlobal\\imperial_college_visualization\\01_data"
  localbig="C:\\Users\\rmart\\OneDrive - Imperial College London\\inglobe"
}


if (Sys.info()["nodename"] == "MS-RMARTIN-WS2" ) {
  #datad="C:\\Users\\rmartin7\\OneDrive - WBG\\projects\\PRINZGlobal\\imperial_college_visualization\\01_data"
...
# Find Dropbox folder - checks common locations on Windows
find_dropbox <- function() {
  # Check for Dropbox info file (most reliable method)
  info_paths <- c(
  ...
  for (info_path in info_paths) {
    if (file.exists(info_path)) {
      ...
      if (!is.null(info$personal$path)) return(info$personal$path)
      if (!is.null(info$business$path)) return(info$business$path)
    }
  }
  ...
  common_paths <- c(
    file.path(user_home, "Dropbox"),
    file.path(user_home, "Dropbox (Personal)"),
    file.path(user_home, "Dropbox (Business)"),
    "C:/Dropbox",
    "D:/Dropbox"
  )
  ...
  warning("Dropbox folder not found")
  return(NULL)
}

dropbox <- find_dropbox()
cat("Dropbox found at:", dropbox, "\n")
...
file_path <- file.path(dropbox, "patbis2021", "data", "fromPATSTAT", "citenet.dsv")
# Using data.table's fread (fast and auto-detects delimiter)
library(data.table)
citenet<- fread(file_path)

# get latest nodes
file_path <- file.path(dropbox, "patbis2021",
                       "data", "fromWATSON",
                       "innos_ev_nationalkey_2009_2018.dsv")
# Using data.table's fread (fast and auto-detects delimiter)
innos_national<- fread(file_path)


# get latest nodes
file_path <- file.path(dropbox, "patbis2021",
                       "data", "fromWATSON",
                       "innos_ev_global_2009_2018.dsv")
# Using data.table's fread (fast and auto-detects delimiter)
innos_global<- fread(file_path)


innos_global %>% distinct(docdb_family_id) %>% nrow()

inno_ids=innos_national %>% distinct(docdb_family_id) #%>% nrow()

citenet=citenet %>% inner_join(inno_ids) %>% 
  bind_rows(citenet %>% 
              inner_join(inno_ids %>%
                         rename(cited_docdb_family_id=docdb_family_id))) %>% 
  distinct()

citenet %>% write_fst(file.path(dropbox, "Apps",
                       "iseapp", "inglobe",
                       "citenet.fst"),compress=100)
...
countrymap=dropbox %>% paste0("/apps/iseapp/countrymap.fst") %>% read_fst()
countrymap_GB=countrymap %>% filter(ctry_code=="GB")

# applications mapped to person....

anti_persons=countrymap_GB %>% anti_join(bind_rows(inventors,holders))
pro_persons=countrymap_GB %>% inner_join(bind_rows(inventors,holders))
pro_persons=pro_persons %>% distinct(person_id)
pro_persons_GB=pro_persons %>% inner_join(df_GB) %>% distinct(person_id)
pro_persons_GB %>% names()

inventors %>% names()


#df_GB_with_nuts1.rds
nuts=readRDS( paste0(localbig, "\\data\\df_GB_with_nuts1.rds"))
nuts_cool=nuts %>% filter(!is.na(nuts1_code))


nuts_cool_pro_personsGB=nuts_cool %>% inner_join(pro_persons_GB)


priority=pro_persons_GB
write_fst(priority,paste0(localbig, "\\data\\priority.fst"))





priority_with_nuts=read_fst( paste0(localbig, "\\data\\priority_with_nuts1.fst"))
priority_with_nuts %>% names()
priority_with_nuts %>% filter(!is.na(nuts1_code)) %>% nrow()

#priority_with_nuts %>% filter(!is.na(nuts1_code)) %>% nrow()

# Check in with maxwell file...
#regionmap_maxwell=dropbox %>% paste0("/apps/iseapp/regionmap.fst") %>% read_fst()
#write_fst(regionmap_maxwell, paste0(dropbox , "/apps/iseapp/regionmap_maxwell.fst"))
#regionmap_maxwell %>% names()

# build new regionmaps
countrymap %>% names()
priority_with_nuts %>% names()
inventors %>% names()
...
regionmap=priority_with_nuts %>%  filter(!is.na(nuts1_code)) %>% 
          inner_join( inventorsANDholders) 

regionmap=regionmap %>%           inner_join(countrymap_GB  %>%  distinct(docdb_family_id,appln_id) ) 
#test %>% distinct(docdb_family_id) %>% nrow()
  
regionmap=regionmap %>% rename(region_code=nuts1_code,region_name=nuts1_name)  %>% mutate(ctry_code="GB")
regionmap %>% distinct(docdb_family_id) %>% nrow()

regionmap=regionmap %>% select(region_code,region_name,docdb_family_id,ctry_code,appln_id) %>% distinct()

write_fst(regionmap, paste0(dropbox , "/apps/iseapp/regionmap.fst"))

regionmap=read_fst( paste0(dropbox , "/apps/iseapp/regionmap.fst"))
regionmap %>% names()
...
dfnew = read_parquet(file.path(dropbox, "Apps",
                       "iseapp", "inglobe",
                       "dfnew.parquet"))


dfnew %>% names()
innos_national %>% names()

#Add country to inventors and holders
...
result <- inventorsANDholders[sample(nrow(inventorsANDholders)), ] |>
  fgroup_by(ctry_code, docdb_family_id) |>
  ffirst()

longlats=innos_national %>% select(ctry_code,docdb_family_id) %>%
  left_join(result %>% select(ctry_code,docdb_family_id,longitude,latitude))

# Fill missing longitude/latitude with capital city coordinates
...
# Join with capital coordinates and fill missing values
longlats <- longlats %>%
  left_join(capital_coords, by = "ctry_code") %>%
  mutate(
    longitude = ifelse(is.na(longitude), capital_lon, longitude),
    latitude = ifelse(is.na(latitude), capital_lat, latitude)
  ) %>%
...
write_fst(longlats, paste0(dropbox , "/apps/iseapp/inglobe/longlats.fst"))
...
library(fst)
filen=paste0(localbig,"\\data\\iseappfirst_appli.fst")
iseappfirst_appli=read_fst(filen)
...
filen=paste0(localbig,"\\data\\appln_docdb.fst")
...
dfnew_GB=dfnew %>% ungroup() %>% fsubset(person_ctry_code=="GB")
