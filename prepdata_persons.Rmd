...
## only persond_id and han_id
```{r}
filen=paste0(inglobebig,"\\persons_han_only.parquet")
if (!file.exists(filen)) {
  library(bigrquery)
  library(DBI)
...
### Get top technology companies by R&D spending (EU Industrial R&D Investment Scoreboard)

We download **both** the 2025 cross-section (for sector classification) and the
full panel (2003–2024) so we can:
- use the combined list of companies across all years for alias matching
- compute total and average R&D spending over the whole period
...
# --- 2025 cross-section (used for sector / ICB3 classification) ---
...
if (!file.exists(top100file)) {
  ...
  unlink(tmpfile)
  message("Cross-section columns: ", paste(names(sb), collapse = ", "))
  write_parquet(sb, top100file)
}
top_rd <- read_parquet(top100file)

# --- Full panel (2003–2024) ---
panel_file <- paste0(inglobebig, "\\scoreboard_panel.parquet")
if (!file.exists(panel_file)) {
  panel_url <- "https://iri.jrc.ec.europa.eu/sites/default/files/contentype/scoreboard/2025-12/Scoreboard_panel_2025.xlsx"
  tmpfile <- tempfile(fileext = ".xlsx")
  download.file(panel_url, tmpfile, mode = "wb")
  panel_raw <- read_excel(tmpfile, sheet = 1)
  unlink(tmpfile)
  message("Panel columns: ", paste(names(panel_raw), collapse = ", "))
  message("Panel rows: ", nrow(panel_raw))
  write_parquet(panel_raw, panel_file)
}
scoreboard_panel <- read_parquet(panel_file)
message("Panel: ", nrow(scoreboard_panel), " rows, ", ncol(scoreboard_panel), " cols")

# --- Build combined unique company list from panel + cross-section ---
# The panel may use a different company column name; detect it
panel_names <- names(scoreboard_panel)
panel_company_col <- intersect(c("Company", "company", "Name", "name",
                                  "Company name", "Company.name"), panel_names)
if (length(panel_company_col) == 0) {
  panel_company_col <- panel_names[sapply(scoreboard_panel, is.character)][1]
} else {
  panel_company_col <- panel_company_col[1]
}

# Canonicalize company names: strip legal suffixes that cause near-duplicates
# across panel years (e.g. "4D MOLECULAR THERAPEUTICS" vs
# "4D MOLECULAR THERAPEUTICS, INC."). Same regex used later for alias cleaning.
clean_company_raw <- function(x) {
  x %>%
    toupper() %>%
    str_replace_all("[.,;:!?'\"\\-]", " ") %>%
    str_replace_all(
...
# Combined unique company names across all panel years + 2025 cross-section
# Apply clean_company_raw to canonicalize before deduplication
all_scoreboard_companies <- unique(clean_company_raw(c(
  scoreboard_panel[[panel_company_col]],
  top_rd$Company
)))
all_scoreboard_companies <- all_scoreboard_companies[!is.na(all_scoreboard_companies) &
                                                      nchar(all_scoreboard_companies) >= 2]
message("Combined unique companies across all years (after cleaning): ",
        length(all_scoreboard_companies))
...
...actually no longer true...let's combine all...
...
if (!file.exists(aliasfile)) {
  ...
  # Use the combined list of companies from all panel years + 2025 cross-section
  companies <- all_scoreboard_companies
  ...
  for (i in seq_along(batches)) {
    ...
    prompt <- paste0(
      ...
      "- Abbreviated forms as they commonly appear in patent office records ",
      "(e.g. 'Elect' for Electronics, 'Int' for International, 'Prop' for ",
      "Property, 'Amer' for America/American, 'Tech' for Technology)\n\n",
...
if (!file.exists(matchfile)) {
  ...
  scoreboard_person_mapping <- bq_table_download(tb,page_size=50000)
...
all_companies       <- all_scoreboard_companies
...
# Upload to BigQuery
bq_table_upload(
  x = bq_table(project_id, dataset, "scoreboard_person_mapping"),
  values = scoreboard_person_mapping,
  write_disposition = "WRITE_TRUNCATE"
)
message("Uploaded ", nrow(scoreboard_person_mapping), " rows to patbis.inglobe.scoreboard_person_mapping")

```


#### Step 4b: LLM reverse-matching of large unmatched han_names

The alias-based matching (Steps 1–4) misses cases where PATSTAT `han_name`
uses its own abbreviation conventions (e.g. "Elect" for ELECTRONICS,
"Int" for INTERNATIONAL). Here we identify large unmatched entities
(≥500 families) and ask the OpenAI API which scoreboard company
each one belongs to.
...
llm_matchfile <- paste0(inglobebig, "\\llm_reverse_matches.parquet")

if (!file.exists(llm_matchfile)) {

  # 1. Find large unmatched han_name entities via BigQuery
  large_unmatched_sql <- "
    WITH scoreboard_han_ids AS (
      SELECT DISTINCT p.han_id
      FROM `patbis.inglobe.scoreboard_person_mapping` spm
      INNER JOIN `patbis.fromPATSTAT2021.tls206_person` p
        ON spm.person_id = p.person_id
      WHERE p.han_id IS NOT NULL
    ),
    holder_han AS (
      SELECT p.han_id, p.han_name, h.docdb_family_id
      FROM `patbis.inglobe.holders` h
      INNER JOIN `patbis.fromPATSTAT2021.tls206_person` p
        ON h.person_id = p.person_id
      WHERE p.psn_sector = 'COMPANY'
        AND p.han_name IS NOT NULL
        AND TRIM(p.han_name) != ''
    ),
    han_counts AS (
      SELECT han_id, han_name, COUNT(DISTINCT docdb_family_id) AS n_families
      FROM holder_han
      GROUP BY han_id, han_name
      HAVING COUNT(DISTINCT docdb_family_id) >= 500
    )
    SELECT hc.*
    FROM han_counts hc
    LEFT JOIN scoreboard_han_ids si ON hc.han_id = si.han_id
    WHERE si.han_id IS NULL
    ORDER BY n_families DESC
  "

  message("Finding large unmatched han_name entities...")
  tb_unmatched <- bq_project_query(project_id, large_unmatched_sql)
  large_unmatched <- bq_table_download(tb_unmatched)
  message("Found ", nrow(large_unmatched), " unmatched han_names with >= 500 families")

  if (nrow(large_unmatched) > 0) {

    # 2. Send to OpenAI in batches
    api_key <- Sys.getenv("OPENAI_API_KEY")
    if (nchar(api_key) == 0) stop("Set OPENAI_API_KEY in .Renviron and restart R")

    # Build the scoreboard company list for context
    sb_list <- paste(all_scoreboard_companies, collapse = "\n")

    batch_size <- 30
    han_names_to_check <- large_unmatched$han_name
    batches <- split(han_names_to_check,
                     ceiling(seq_along(han_names_to_check) / batch_size))

    all_llm_matches <- list()

    for (i in seq_along(batches)) {
      batch <- batches[[i]]
      message("LLM reverse-match batch ", i, " / ", length(batches),
              " (", length(batch), " han_names)")

      han_list <- paste(batch, collapse = "\n")

      prompt <- paste0(
        "Below are two lists:\n\n",
        "LIST A: Entity names from a patent database (PATSTAT han_name). ",
        "These names often use abbreviations (e.g. 'Elect' for Electronics, ",
        "'Int' for International, 'Tech' for Technology, 'Corp' for Corporation, ",
        "'Prop' for Property, 'Amer' for America).\n\n",
        "LIST B: EU R&D Scoreboard company names (official company names).\n\n",
        "For each entity in List A, determine if it clearly corresponds to a ",
        "company in List B (it could be the company itself, a major subsidiary, ",
        "a former name, or a regional variant). Only return matches you are ",
        "confident about.\n\n",
        "Return ONLY a JSON array. Each element should have:\n",
        "  - \"han_name\": the exact name from List A\n",
        "  - \"company_raw\": the exact name from List B that it matches\n",
        "  - \"confidence\": \"high\" or \"medium\"\n",
        "  - \"reason\": brief explanation (e.g. \"subsidiary\", \"abbreviated form\", ",
        "\"former name\")\n\n",
        "If a han_name does not match any scoreboard company, omit it entirely.\n",
        "Return valid JSON only, no markdown, no commentary.\n\n",
        "=== LIST A (patent database entity names) ===\n",
        han_list, "\n\n",
        "=== LIST B (EU R&D Scoreboard companies) ===\n",
        sb_list
      )
      ...
      resp <- request("https://api.openai.com/v1/chat/completions") %>%
        ...
        req_body_json(list(
          ...
          messages    = list(
            list(role = "system",
                 content = paste0(
                   "You are an expert on multinational corporations, their ",
                   "subsidiaries, and how they appear in patent databases. ",
                   "You are familiar with PATSTAT name harmonization conventions. ",
      ...
      parsed <- tryCatch(
        fromJSON(reply, simplifyDataFrame = TRUE),
        error = function(e) {
          message("  JSON parse error in batch ", i, ": ", e$message)
          message("  Raw reply (first 500 chars): ", substr(reply, 1, 500))
          return(NULL)
      ...
      if (!is.null(parsed) && nrow(parsed) > 0) {
        all_llm_matches[[i]] <- parsed
        message("  Found ", nrow(parsed), " matches in batch ", i)
      }

      Sys.sleep(2)  # respect rate limits
    }

    llm_matches <- bind_rows(all_llm_matches)

    if (nrow(llm_matches) > 0) {
      message("Total LLM matches: ", nrow(llm_matches))
      print(llm_matches)

      # Save raw LLM output
      write_parquet(llm_matches, llm_matchfile)

      # 3. Filter for high-confidence matches only
      high_conf <- llm_matches %>% filter(confidence == "high")
      message("High-confidence matches: ", nrow(high_conf))

      # Verify company_raw values actually exist in scoreboard list
      high_conf <- high_conf %>%
        filter(company_raw %in% all_scoreboard_companies)
      message("Verified matches (company_raw in scoreboard): ", nrow(high_conf))

      if (nrow(high_conf) > 0) {

        # 4. Look up han_id for each matched han_name
        llm_han <- large_unmatched %>%
          inner_join(high_conf, by = "han_name") %>%
          select(company_raw, han_id, han_name) %>%
          distinct()

        # Upload to BigQuery for expansion
        bq_table_upload(
          x = bq_table(project_id, dataset, "llm_han_matches"),
          values = llm_han,
          write_disposition = "WRITE_TRUNCATE"
        )
        ...
        expand_llm_sql <- "
          SELECT DISTINCT
            f.company_raw,
            f.han_name AS alias,
            'llm_reverse' AS match_type,
            f.han_id,
            f.han_name,
            p.person_id,
            p.person_name,
            p.person_ctry_code
          FROM `patbis.inglobe.llm_han_matches` f
          INNER JOIN `patbis.fromPATSTAT2021.tls206_person` p
            ON f.han_id = p.han_id
          INNER JOIN (
            SELECT DISTINCT person_id
            ...
            ON p.person_id = h.person_id
        "
        tb_llm <- bq_project_query(project_id, expand_llm_sql)
        llm_expanded <- bq_table_download(tb_llm)

        message("LLM matches expanded to ", nrow(llm_expanded), " person_id rows")

        # 5. Merge into main mapping
        scoreboard_person_mapping <- bind_rows(
          scoreboard_person_mapping,
          llm_expanded
        ) %>% distinct()

        # Re-save locally and to BigQuery
        write_parquet(scoreboard_person_mapping, matchfile)
        bq_table_upload(
          x = bq_table(project_id, dataset, "scoreboard_person_mapping"),
          values = scoreboard_person_mapping,
        ...
        message("Updated mapping saved: ", nrow(scoreboard_person_mapping),
                " total rows (including LLM reverse matches)")
      }
    } else {
      message("No LLM matches found")
      write_parquet(tibble(han_name = character(), company_raw = character(),
                           confidence = character(), reason = character()),
                    llm_matchfile)
    }
  } else {
    message("No large unmatched entities found — skipping LLM step")
    write_parquet(tibble(han_name = character(), company_raw = character(),
                         confidence = character(), reason = character()),
                  llm_matchfile)
  }
} else {
  message("LLM reverse matches already cached: ", llm_matchfile)
  llm_matches <- read_parquet(llm_matchfile)
  message("  ", nrow(llm_matches), " matches on file")
}
...
firmmap=read_parquet( firmmap_file)
...
#### Step 7: Create firm sector mapping with panel R&D statistics

Map each company to its ICB3 sector classification (from the 2025 cross-section)
and compute total and average R&D spending across all panel years (2003–2024).
...
# --- Sector from the 2025 cross-section ---
...
message("Using sector column: ", icb3_col[1])

# Apply clean_company_raw so sector_lookup uses the same canonical names
sector_lookup <- top_rd %>%
  mutate(company_raw = clean_company_raw(Company)) %>%
  select(company_raw, sector = all_of(icb3_col[1])) %>%
  distinct()

# --- R&D from the full panel ---
# Detect panel structure: long (has year column) or wide (year-based columns)
panel_names <- names(scoreboard_panel)
year_col <- intersect(c("year", "Year", "YEAR", "Fiscal year",
                         "fiscal_year", "fyear"), panel_names)
rd_col <- grep("R&D|R.D|RnD|rd_", panel_names, value = TRUE, ignore.case = TRUE)

if (length(year_col) > 0) {
  # Long format — canonicalize company names to match firmmap
  panel_rd <- scoreboard_panel %>%
    mutate(company_raw = clean_company_raw(.data[[panel_company_col]])) %>%
    rename(year = all_of(year_col[1]),
           RnD  = all_of(rd_col[1])) %>%
    select(company_raw, year, RnD) %>%
    filter(!is.na(company_raw), !is.na(RnD))
} else {
  # Wide format: pivot year-based columns to long — canonicalize company names
  year_rd_cols <- grep("(19|20)\\d{2}", panel_names, value = TRUE)
  panel_rd <- scoreboard_panel %>%
    mutate(company_raw = clean_company_raw(.data[[panel_company_col]])) %>%
    select(company_raw, all_of(year_rd_cols)) %>%
    pivot_longer(cols = all_of(year_rd_cols),
                 names_to = "col_name", values_to = "RnD") %>%
    mutate(year = as.integer(str_extract(col_name, "(19|20)\\d{2}"))) %>%
    filter(!is.na(year), !is.na(RnD)) %>%
    select(company_raw, year, RnD)
}

# Compute total and average R&D per company across all panel years
panel_rd_summary <- panel_rd %>%
  group_by(company_raw) %>%
  summarise(
    RnD_total = sum(RnD, na.rm = TRUE),
    RnD_avg   = mean(RnD, na.rm = TRUE),
    n_years   = n(),
    .groups   = "drop"
  )

# --- Combine sector + R&D stats, restricted to firmmap companies ---
firmmap_companies <- unique(firmmap$company_raw)

firmsectormap <- sector_lookup %>%
  full_join(panel_rd_summary, by = "company_raw") %>%
  filter(company_raw %in% firmmap_companies) %>%
  distinct()

message("Firm sector map: ", nrow(firmsectormap), " companies")
message("  with sector: ", sum(!is.na(firmsectormap$sector)))
message("  with R&D panel data: ", sum(!is.na(firmsectormap$RnD_total)))
...
# Also save the full panel (company x year) for time-series analysis
panel_matched <- panel_rd %>%
  filter(company_raw %in% firmmap_companies) %>%
  left_join(sector_lookup, by = "company_raw")

panel_file_out <- paste0(dropbox_root, "\\Apps\\iseapp\\firmsectormap_panel.parquet")
write_parquet(panel_matched, panel_file_out)
message("Panel saved to ", panel_file_out, " (",
        nrow(panel_matched), " rows, ",
        n_distinct(panel_matched$company_raw), " companies, ",
        "years ", min(panel_matched$year), "-", max(panel_matched$year), ")")
rm(panel_rd, panel_matched); gc()
...
recursive_mapbox=function(){
  ...
  some_addresses=df_filtered %>% head(10000) %>%  mutate(addressstr=paste(person_address,person_ctry_code,sep=","))
