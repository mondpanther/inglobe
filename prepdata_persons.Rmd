...
all_scoreboard_companies <- unique(c(
  scoreboard_panel[[panel_company_col]],
  top_rd$Company
))
all_scoreboard_companies <- all_scoreboard_companies[!is.na(all_scoreboard_companies)]
message("Combined unique companies across all years: ", length(all_scoreboard_companies))
...
if (length(year_col) > 0) {
  ...
  panel_rd <- scoreboard_panel %>%
    rename(company_raw = all_of(panel_company_col),
           year = all_of(year_col[1]),
           RnD  = all_of(rd_col[1])) %>%
    select(company_raw, year, RnD) %>%
    filter(!is.na(company_raw), !is.na(RnD))
} else {
  # Wide format: pivot year-based columns to long
